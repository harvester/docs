...
To override this behavior, set either the kernel parameter `harvester.install.skipchecks=true` or the configuration file option `install.skipchecks=true`. When set to `true`, warning messages are still saved to `/var/log/console.log`, but the installation proceeds even if hardware requirements for production use are not met.
:::
