...
Harvester does not automatically promote a worker node to a control plane node if a control plane node goes offline or fails due to some reason, promotion only occurs when a control plane node is removed from the cluster.
