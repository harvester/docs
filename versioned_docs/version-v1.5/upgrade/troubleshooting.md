...
To provide you with access to the upgrade logs even after the upgrade completes, whether it succeeded or failed, the upgrade log downloader deployment and the log archive volume are kept running in the cluster. They will continue to serve and also occupy cluster resources until you click the **Dismiss it** button or delete the corresponding UpgradeLog custom resource via `kubectl`. Some operations, e.g., updating the storage network settings, can be blocked for this reason. For more details, see [#9599](https://github.com/harvester/harvester/issues/9599).
