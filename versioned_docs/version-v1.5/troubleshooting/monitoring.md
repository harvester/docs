...
## Some PODs of `rancher-monitoring` addon are instantly killed

### Issue Description

After enabling the `rancher-monitoring` addon, the `prometheus, alertmanager, grafana` related pods are keeping killed shortly after they are up.
...
When check the `prometheus` pod log, it might have the keyword `level=warn msg="Received SIGTERM, exiting gracefully..."`.
...
Check the `prometheus` CRD object, it has the `storage-network.settings.harvesterhci.io/replica: "1" ` annotation.
...
Log from Harvester pod via deployment 'harvester-system/harvester' shows the `storage network` change is blocked:
...
When [storage network](../advanced/storagenetwork.md#harvester-storage-network-setting) setting is changed, Harvester controller checks and waits the attached volumes to be detached, then applies the new configuration. Meanwhile, the controller kills `prometheus`, `grafana` and `alertmanager` related pods automatically as those pods are using volumes to store data. Normally this process consists for a short while.

If the [storage network](../advanced/storagenetwork.md#harvester-storage-network-setting) change is unfortunately blocked by some attached volumes, and the user or the monitoring-operator tries to bring up the `rancher-monitoring`, then the pods are killed by the controller quickly, the aforementioined phenomenon is observed.

### Workaround

1. Disable the `rancher-monitoring` addon.

1. Check the current [storage network](../advanced/storagenetwork.md#harvester-storage-network-setting) setting, confirm it is enabled or disabled.

1. Keep checking the logs from Harvester pods, when it still complains some volues are attached, stop the owner VMs. Until it does not report any errors after the message `storage network change`.

1. Enable the `rancher-monitoring` addon.
