...
4. When a VM or storage network is created in Harvester, corresponding veth interfaces connected to the pods are created on the host. In earlier versions, these veth interfaces were associated with both VLAN ID 1 
   and the VLAN ID assigned to the VM Network.This allowed the Harvester bridge to handle both untagged (VLAN 1) and tagged traffic from external switches and forward it correctly to the veth interface.
