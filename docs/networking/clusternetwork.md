...
Any additional vlans required on the `mgmt-br` bridge and `mgmt-bo` must be explicitly configured using `bridge vlan add vid <vlan-id> dev mgmt-br self` and `bridge vlan add vid <vlan-id> dev mgmt-bo` commands on the Harvester host.
