...
When the Harvester cluster is imported to `Rancher Manager`, and you operate from `Rancher Manager UI > Virtualization Management` and locate to this page, the `Network` tab in the IP Pools section looks differently.
...
1. It is better to [Create IP Pool from Rancher Managery UI](#create-ip-pool-from-rancher-manager-ui), because it allows you to tune `Project` and `Guest Kubernetes Cluster` for better resources management and isolation.
