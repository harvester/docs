...
## Longhorn Replicas with a Witness Node

The Longhorn storage system is a highly avaiable distributed storage system that create replicas for volumes. Longhorn is provisioned to management or worker nodes, but not to witness nodes. The default replica number is set to 3. That means in following scenarios, 3 nodes cluster with 1 witness node, Longhorn will not be able to create the default number of replicas. This will cause the potential risk for high availability. Please check on the following section to get more details.

## Limitations

The default StorageClass `harvester-longhorn` of Harvester has a replica count value of `3` for high availability. According to above, if you use this StorageClass to create volumes with 2 nodes + 1 witness node, Longhorn is unable to create the configured number of replicas. This results in volumes being marked as **Degraded** on the Longhorn UI. If your cluster have more than 3 nodes, the volume will be created successfully and **healthy**.
