...
To build out-of-tree kernel modules, configure a DaemonSet that compiles the required modules in an `initContainer` and loads them using `modprobe`. The following example demonstrates this concept using DRBD source code from [LINBIT](linbit.com):
...
The following example uses DRBD source code from [LINBIT](linbit.com):
