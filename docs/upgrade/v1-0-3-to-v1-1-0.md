...
Once there is an upgradable version, the Harvester GUI Dashboard page will have an upgrade button. For more details, please refer to [start an upgrade](./automatic.md#start-an-upgrade).
...
Starting from v1.1.0, Harvester will wait for all volumes to become healthy (when node counts >= 3) before upgrading a node. Generally, users can check volumes' health if an upgrade is stuck with the "pre-draining" state.
...
An upgrade is stuck, like the screenshot:
