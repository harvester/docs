...
This occurs when you upgrade harvester from old vesrions, including `v1.0.x`, `v1.1.x` and `v1.2.x`.
...
The root cause is that we didn't apply the latest `depdendency_charts` CRDs because Helm doesn't manage CRDs for us. In order to make upgrade continue, please execute following script. It includes `kubevirt`, `csi-snapshotter` and `whereabouts`.
...
After 5 minutes, please check status in `mcc-harvseter` of `bundle.fleet.cattle.io/v1alpha1`. If it still shows the same error, we need to force bundle to resync it.
