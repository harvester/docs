...
The root cause is that we didn't apply the latest `depdendency_charts` CRDs because Helm doesn't manage CRDs for us. In order to make upgrade continue, please execute folloing script. It includs `kubevirt`, `csi-snapshotter` and `whereabouts`.
