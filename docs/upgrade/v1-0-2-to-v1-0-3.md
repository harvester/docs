...
### 2. Upgrade is stuck, a node is in "Pre-drained" state
...
This might be caused by instance-manager-r pod on node `harvester-z7j2g` can’t be drained. To verify:
...
- Check degraded volumes:
    ...
    **NOTE:** the user needs to check all degraded volumes one by one.
...
- Restart the backing-image-manager:
